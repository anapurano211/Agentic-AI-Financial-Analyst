...
def fetch_all_metrics(symbols, rsi_period=21):
    ...
    # Build each block with batching pauses
    profile_blocks, quote_blocks, ratios_blocks, km_blocks = {}, {}, {}, {}
    for i, s in enumerate(symbols, start=1):
        ...
        time.sleep(0.02)     # light jitter
        _batch_pause(i)      # batch pause
...
with st.sidebar:
    ...
    cap_b_min, cap_b_max = st.slider("Market Cap Range ($B)", 0, 8000, (0, 8000), 1)
    volume_min, volume_max = st.slider("Avg Volume Range", 0, 50_000_000, (50_00_000, 10_000_000), 100_000)
    ...
    if run_step2:
        ...
        if s1["include_transcripts"]:
            with st.spinner("Fetching & summarizing earnings transcripts..."):
                for i, t in enumerate(prof["ticker"], start=1):
                    ...
                    if not meta_t or not meta_t.get("content"):
                        ...
                        time.sleep(0.02); _batch_pause(i)
                        continue
